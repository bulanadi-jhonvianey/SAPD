...
<body>
    ...
    <div class="sidebar" id="sidebar">
        <div class="sidebar-content">
            <ul class="nav flex-column">
                ...
                <li class="nav-item"><a class="nav-link" href="employee_form.php"><i class="fas fa-car me-3"></i>
                        Employee Parking Form</a></li>
                <li class="nav-item"><a class="nav-link" href="student_form.php"><i class="fas fa-car-side me-3"></i>
                        Student Parking Form</a></li>
    ...
    <div class="modal fade" id="activeUsersModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-footer"><a href="active_users.php" class="btn btn-primary rounded-pill px-4">Full
    ...
    <div class="modal fade" id="pendingRequestsModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-footer"><a href="admin_approval.php"
    ...
    <div class="modal fade" id="guidanceModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-footer"><a href="guidance_referral.php"
    ...
    <div class="modal fade" id="parkingModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header border-bottom">
                    <h5 class="modal-title fw-bold text-success"><i class="fas fa-car-side me-2"></i>Recent Employee
                        Parking Applications</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="p-3 sticky-top modal-search-container">
                    <div class="input-group"><span class="input-group-text input-group-text-themed"><i
                            ...
                            <div class="modern-list-item">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="list-info">
                                        ...
                                        <div class="list-subtitle"><span><i class="fas fa-id-card me-1"></i>Employee
    ...
    <div class="modal fade" id="studentParkingModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header border-bottom">
                    <h5 class="modal-title fw-bold text-info"><i class="fas fa-car me-2"></i>Recent Student Parking
                        Applications</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="p-3 sticky-top modal-search-container">
                    <div class="input-group">
                        <span class="input-group-text input-group-text-themed"><i class="fas fa-search"></i></span>
                        <input type="text" id="studentParkingSearch"
                            class="form-control form-control-themed border-start-0" placeholder="Filter by Name...">
                    </div>
                </div>
                        ...
                        <?php foreach ($recent_student_parking as $p): ?>
                            <div class="modern-list-item">
                                ...
                                <div class="text-end">
                                    <div class="text-muted small mb-1"><?php echo $p['created_at'] ?? date('Y-m-d'); ?></div>
                                    <span
                                        class="modern-badge badge-soft-info"><?php echo htmlspecialchars($p['status'] ?? 'Submitted'); ?></span>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php else: ?>
    ...
    <script>
        ...
        // --- FULLCALENDAR SETUP & EVENT SAVING LOGIC ---
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function (info) {
                    // Set up form to EDIT an existing event
                    document.getElementById('eventId').value = info.event.id;
                    document.getElementById('eventTitle').value = info.event.title;

                    // Extract times
                    const startStr = info.event.start ? new Date(info.event.start.getTime() - (info.event.start.getTimezoneOffset() * 60000)).toISOString().slice(0, 16) : '';
                    const endStr = info.event.end ? new Date(info.event.end.getTime() - (info.event.end.getTimezoneOffset() * 60000)).toISOString().slice(0, 16) : startStr;

                    document.getElementById('eventStart').value = startStr;
                    document.getElementById('eventEnd').value = endStr;
                    document.getElementById('eventColor').value = info.event.backgroundColor || '#4e73df';
            ...
            document.getElementById('saveEventBtn').addEventListener('click', function (e) {
                ...
                const eventData = { id: id, title: title, start: start, end: end, color: color };

                // Send request to your backend handler
                fetch('event_handler.php?action=save', {
                    method: 'POST',
                    ...
                    .catch(error => {
                        // FALLBACK: If backend isn't ready yet, add it visually to the calendar anyway
                        console.log("Saving locally as fallback...", error);
                        if (id) {
            ...
            document.getElementById('deleteEventBtn').addEventListener('click', function (e) {
                ...
                if (confirm("Are you sure you want to delete this event?")) {
                    fetch('event_handler.php?action=delete', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        ...
                        .then(response => {
                            let existingEvent = calendar.getEventById(id);
                            if (existingEvent) existingEvent.remove(); // Visually remove it
                            modal.hide();
                        })
                        .catch(error => {
